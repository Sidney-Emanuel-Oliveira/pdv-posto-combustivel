    ...
    public ContatoList(ContatoService service) {
        ...
        btnEditar.addActionListener(e -> {
            ...
            if (selectedRow != -1) {
                ...
                if (selectedRow < contatos.size()) {
                    Contato contato = contatos.get(selectedRow);
                    if (contato.getId() != null) {
                        ContatoForm form = new ContatoForm(contatoService, this, contato);
                        form.setVisible(true);
                    } else {
                        JOptionPane.showMessageDialog(this, "Contato não possui ID válido!");
                    }
                }
            } else {
        ...
        btnRemover.addActionListener(e -> {
            int selectedRow = table.getSelectedRow();
            if (selectedRow != -1) {
                // Pegar o contato da lista usando o índice da linha
                List<Contato> contatos = contatoService.getAllContatos();
                if (selectedRow < contatos.size()) {
                    Contato contato = contatos.get(selectedRow);
                    if (contato.getId() != null) {
                        contatoService.removeContato(contato.getId());
                        atualizarTabela();
                    } else {
                        JOptionPane.showMessageDialog(this, "Contato não possui ID válido!");
                    }
